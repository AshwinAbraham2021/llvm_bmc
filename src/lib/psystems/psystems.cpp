...
// unsigned psystems::get_word_size(const llvm::Value* v ) {
...
//   }else{
//     llvm_bmc_error("psystems", "size of global variable is unkown!");
//   }
// }

// svec psystems::get_init_array(const llvm::Value* v, unsigned size ) {
//   svec init;
...
// std::string psystems::read_const( const llvm::Value* v ) {
//   if( auto gv = llvm::dyn_cast<const llvm::GlobalVariable>(v) ) {
...
// std::string psystems::get_reg( const llvm::Value* v, svec& idxs ) {
//   auto s = read_const(v);
//   if( s != "" ) {
...
// std::string psystems::get_reg_time( const llvm::Value* v, svec& idxs) {
//   auto s = read_const(v);
//   if( s != "" ) {
//     assert( idxs.size() == 0 );
//     return "0";
...
// std::string psystems::get_reg_time( const llvm::Value* v) {
//   svec idxs;
...
// bool psystems::is_acquire( llvm::AtomicOrdering ord ) {
...
//   default:
//     llvm_bmc_error( "psystems", "unrecognized C++ ordering returned!!" );
//   }
//   return false; // dummy return;
// }

// bool psystems::is_release( llvm::AtomicOrdering ord ) {
//   // return hb_enc::o_tag_t::sc; // test return;
//   switch( ord ) {
//   case llvm::AtomicOrdering::NotAtomic: return false; break;
//   case llvm::AtomicOrdering::Unordered: return false; break;
...
// void psystems::dump_CmpInst( unsigned bidx, const llvm::CmpInst* cmp) {
//   assert( cmp );
...
//   default: {
//     llvm_bmc_error("psystems","unsupported predicate in compare " <<pred<< "!!");
//   }
//   }
...
// void psystems::dump_ExtractValue( const llvm::ExtractValueInst* eval) {
...
// void psystems::dump_IntrinsicInst( unsigned bidx,
...
//   }
//   else{
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( ConstrainedFPIntrinsic, I);
// #ifndef LLVM_SVN
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( AtomicMemCpyInst, I);
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( AtomicMemMoveInst, I);
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( AtomicMemSetInst, I);
// #endif
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( MemIntrinsic, I);
//     // sub class
//     // PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( MemSetInst, I);
//     // PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( MemTransferInst, I);
//     // PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( MemCpyInst, I);
//     // PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( MemMoveInst, I);
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( VAStartInst, I);
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( VAEndInst, I);
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( VACopyInst, I);
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( InstrProfIncrementInst, I);
//     // PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( InstrProfIncrementInstStep, I);
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( InstrProfValueProfileInst, I);
//     //I->print( llvm::outs() );
//     llvm_bmc_error("bmc", "Unsupported intrinsics!");
//   }
// }

// std::string psystems::get_GEPOperator(const llvm::GEPOperator* gep) {
//   assert( gep );
//   std::string index;
//   std::string scale = "1";
...
// void psystems::dump_GetElementPtrInst( const llvm::GetElementPtrInst* gep) {
//   bool isLocalUse;
...
// void psystems::addr_name( const llvm::Value* addr,
...
//     addr->dump();
//     llvm_bmc_error("psystems", "constant access to the memory!");
//   }
...
// void psystems::dump_FenceInst( const llvm::FenceInst* fence ) {
...
// void psystems::dump_AtomicRMWInst( const llvm::AtomicRMWInst* rmw ) {
//   //get address id
...
//   default:
//     llvm_bmc_error("psystems","unspported atomic rmw operation");
//   }

//   dump_st( w, cw, caddr, gid, is_release(ord), true);
// }

// void psystems::dump_AtomicCmpXchgInst( const llvm::AtomicCmpXchgInst* cxng ) {
//   //get address id
...
// void psystems::dump_CallAssert(unsigned bidx, const llvm::CallInst* call) {
//   assert(call);
//   auto term = get_reg( call->getArgOperand(0) );
//   in_code_spec.push_back( term );
// }

// void psystems::dump_CallNondet(unsigned bidx, const llvm::CallInst* call) {
//   assert(call);
//   auto term = get_reg( call->getArgOperand(0) );
...
//   }else{
//     LLVM_DUMP(call);
//     llvm_bmc_error("psystems", "function call is not recognized !!");
//   }
// }

// void psystems::dump_CallThreadCreate( unsigned bidx, const llvm::CallInst* call){
//   unsigned j = 0;
//   for (; j < bmc_obj.sys_spec.threads.size(); j++) {
...
// void psystems::dump_CallThreadJoin( unsigned bidx, const llvm::CallInst* call){
//   unsigned j = 0;
//   for (; j < bmc_obj.sys_spec.threads.size(); j++) {
...
// void psystems::dump_CastInst( unsigned bidx, const llvm::CastInst* cast ) {
...
// void psystems::dump_AllocaInst( const llvm::AllocaInst* alloc ) {
//   // std::string gid, caddr;
...
// void psystems::dump_LoadInst( unsigned bidx, const llvm::LoadInst* load ) {
//   assert( load );
...
//   }else{
//     load->dump();
//     llvm_bmc_error("psystems", "we need to support local global optimization!!");
...
// void psystems::dump_LD_(unsigned bidx, const llvm::CallInst* call,
...
//   } else {
//     PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( VAArgInst,        I );
//     // PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( ExtractValueInst, I );
//     LLVM_DUMP(I);
...
//   //   // phi->getParent()->dump();
//   //   llvm_bmc_error("psystems", "phi nodes with non integers not supported !!");
//   // }
...
// void psystems::dump_PhiNodes( const bb* b, const bb* prev_b ) {
...
// void psystems::dump_SelectInst( const llvm::SelectInst *sel ) {
//   assert( sel );
...
// void psystems::dump_SwitchInst( unsigned bidx,
...
// void psystems::dump_UnreachableInst( unsigned bidx,
...
//       PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( IndirectBrInst,    I );
//       //PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( InvokeInst,        I );
//       PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( ResumeInst,        I ); // passing exception to caller
//       PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( CatchSwitchInst,   I );
//       PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( CatchReturnInst,   I );
//       PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( CleanupReturnInst, I );

//       //Other unsupported instructions
//       PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( FuncletPadInst,     I);
//       // todo: cases for funclet CleanupPadInst, CatchPadInst
//       PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( BinaryOperator,     I);
//       // PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( FenceInst,          I);
//       // PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( AtomicCmpXchgInst,  I);
//       // PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( AtomicRMWInst,      I);
//       // PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( SelectInst,         I);
//       PSYSTEMS_UNSUPPORTED_INSTRUCTIONS( ExtractElementInst, I);
